package com.luck.slideleftfinish.twosolutionweight;
...
public class GestureViewGroup extends ViewGroup {
    ...
    private float lastX;
    private float lastY;
    private boolean isSlideFinish = true;
        ...
        if (isSlideFinish) {
            ...
            switch (ev.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE:
                    int distanceX = (int) (ev.getX() - lastX);
                    int distanceY = (int) Math.abs(ev.getY() - lastY);
                    if (Math.abs(distanceX) > distanceY && distanceX > 10) {//处理左滑动
                        result = true;
                    }
    ...
    public void setCanSlideFinish(boolean isCan) {
        isSlideFinish = isCan;
    }
